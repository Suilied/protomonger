...
namespace RVO {
	...
	RVOSimulator::RVOSimulator(float timeStep, float neighborDist, size_t maxNeighbors, float timeHorizon, float timeHorizonObst, float radius, float flockingRadiusScale, float maxSpeed, const Vector2 &velocity) : defaultAgent_(NULL), globalTime_(0.0f), kdTree_(NULL), timeStep_(timeStep)
	{
		kdTree_ = new KdTree(this);
		...
		defaultAgent_->flocking_radius_scale_ = flockingRadiusScale;
		defaultAgent_->flocking_radius_ = neighborDist * flockingRadiusScale;
	...
	size_t RVOSimulator::addAgent(const Vector2 &position)
	{
		...
		agent->flocking_radius_scale_ = defaultAgent_->flocking_radius_scale_;
		agent->flocking_radius_ = defaultAgent_->flocking_radius_;
	...
	size_t RVOSimulator::addAgent(const Vector2 &position, float neighborDist, size_t maxNeighbors, float timeHorizon, float timeHorizonObst, float radius, float flockScale, float maxSpeed, const Vector2 &velocity)
	{
		Agent *agent = new Agent(this);
		...
		agent->flocking_radius_scale_ = flockScale;
		agent->flocking_radius_ = flockScale * radius;
	...
	void RVOSimulator::removeObstacle(const size_t obstacleId) {
		const Obstacle* obstacle_to_remove = obstacles_.at(obstacleId);
		if (obstacle_to_remove != NULL) {
			obstacle_to_remove->prevObstacle_->nextObstacle_ = obstacle_to_remove->nextObstacle_;
			obstacle_to_remove->nextObstacle_->prevObstacle_ = obstacle_to_remove->prevObstacle_;
			obstacles_.erase(obstacles_.begin() + obstacleId);
		}
	}
...
#pragma omp parallel for
	...
	Agent* RVOSimulator::getAgentInPoint(Vector2 point) {
	...
	void RVOSimulator::getAgentsInRectangle(Vector2 topleft, Vector2 bottomright, std::vector<Agent*>* agents) {
		// if the kdTree hasn't kicked in yet, do it live
		if (kdTree_->agents_.size() == 0) {
	...
	void RVOSimulator::setAgentDefaults(float neighborDist, size_t maxNeighbors, float timeHorizon, float timeHorizonObst, float radius, float flockScale, float maxSpeed, const Vector2 &velocity)
	{
		if (defaultAgent_ == NULL) {
			defaultAgent_ = new Agent(this);
		}
		...
		defaultAgent_->neighborDist_ = neighborDist;
		defaultAgent_->radius_ = radius;
		defaultAgent_->flocking_radius_scale_ = flockScale;
		defaultAgent_->flocking_radius_ = neighborDist * flockScale;
		defaultAgent_->timeHorizon_ = timeHorizon;
		defaultAgent_->timeHorizonObst_ = timeHorizonObst;
		defaultAgent_->velocity_ = velocity;
	}
	...
	void RVOSimulator::setAgentSeekTarget(size_t agentNo, const Vector2& seekTargetPos) {
		agents_[agentNo]->seek_target_ = seekTargetPos;
	}

	void RVOSimulator::setAgentFlocking(size_t agentNo, bool doFlock) {
		agents_[agentNo]->do_flocking_ = doFlock;
	}
